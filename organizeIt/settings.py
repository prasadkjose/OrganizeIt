...
def load_yaml_config(yaml_dir: str) -> list[any]:
    ...
    This method reads all YAML files from the directory specified by `yaml_dir`, parses each YAML file, 
    and appends the parsed content to a list, which is then returned.

    Args:
        yaml_dir (str): The directory containing the YAML files to load.

    Returns:
        list[any]: A list containing the parsed contents of all the YAML files found in the specified directory.
            The content can be any Python object depending on the structure of the YAML files.
